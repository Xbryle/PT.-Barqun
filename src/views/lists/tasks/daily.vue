  ...
  <div style="width: 100%; position: inline; overflow: visible !important;">
    ...
    <v-dialog v-model="showDialog" max-width="600px">
      ...
      <v-container class="details-pop" v-for="task in details" :key="task.id">
        <v-subheader class="px-0 d-flex mb-n4">
          <h4 style="font-weight: 500" class="mr-3">Daily</h4>
          <v-spacer></v-spacer>
        ...
        <v-subheader class="mt-n5 py-0 px-0">
          <h4
            style="font-weight: 600; margin-top: -1px; font-size: 0.75rem"
            class="mr-2"
          >
            Start at {{ startDate }}
          </h4>
    ...
    <v-col cols="12" class="mt-n5 outer-add-task">
      ...
      >
        ...
        <input
          type="text"
          v-model="newTaskTitleDaily"
          @keydown="addDaily($event)"
      ...
      >
        <div class="d-flex" style="position: relative">
          ...
          <div style="position: relative">
            ...
            <div class="d-flex" style="justify-content: end; align-items: center; padding-right: 15px;">
              <i
...
export default {
  ...
  // mount
  truncatedTitle() {
    this.filtered = this.dailys.filter((task) => task.title.length);
  ...
  sortTasksMount() {
    const pinnedTasks = this.dailys.filter((task) => task.is_pinned === true || task.is_pinned === 'true');
    const undoneTasks = this.dailys.filter((task) => !task.is_pinned === true && task.status === "undone" || !task.is_pinned === true && task.status === "none");
    const pendingTasks = this.dailys.filter((task) => !task.is_pinned === true && task.status === "pending" || !task.is_pinned === 'true' && task.status === "pending");
    const cancelTasks = this.dailys.filter((task) => !task.is_pinned === true && task.status === "cancel" || !task.is_pinned === 'true' && task.status === "cancel");
    const failedTasks = this.dailys.filter((task) => !task.is_pinned === true && task.status === "failed" || !task.is_pinned === 'true' && task.status === "failed");
    const doneTasks = this.dailys.filter((task) => !task.is_pinned === true && task.status === "done" || !task.is_pinned === 'true' && task.status === "done" || !task.is_pinned === 'true' && task.status === "completed" || !task.is_pinned === true && task.status === "completed");
  ...
  addDailyTask() {
    ...
    axios
      .post(
        `http://127.0.0.1:8080/api/tasks?filter[interval]=daily`,
        { title: this.newTaskTitleDaily, interval: "daily" },
      ...
      .then((response) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            setTimeout(() => {
              const biggestIdData = this.dailys.sort((a, b) => b.id - a.id)[0];
  ...
  pinnedTask(task) {
    ...
    const all = document.getElementById(`task-${task.id}`);
    const findTask = this.dailys.find((tasks) => tasks.id === task.id);
    if(findTask.is_pinned === false) {
      ...
      setTimeout(() => {
        ...
        const index = this.dailys.findIndex((task) => task.id === findTask.id);
        this.dailys.splice(index, 1, updateTask);
        this.truncatedTitle()
        this.sortTasksMount()
  ...
  addSubtask(task) {
    ...
    const getTask = this.dailys.find((tasks) => tasks.id === task.id);
  ...
  deleteSubTask(task, sub) {
    ...
    if (this.assigner === false) {
      ...
      this.massage = "Cannot delete subtask that create by assigner"
      this.snackbar = true;
    }
  },
  setDoneSub(task, sub) {
  ...
  computed: {
    sortedTasks() {
      ...
      const sortedPinned = pinnedTasks.sort((a, b) => {b.id - a.id});
      const sortedUndone = undoneTasks.sort((a, b) => {return new Date(b.start_at) - new Date(a.start_at);});
...
.task-in {
  animation: taskIntro 0.4s ease forwards;
}
@keyframes taskIntro {
  0% {
    z-index: 999;
...
.others-task-in {
  animation: othersTaskIntro 0.4s ease forwards;
}
...
.others-task-out {
  animation: othersTaskOutro 0.4s ease forwards;
}
...
.dailys-intro-animation {
  animation: intro .3s ease forwards;
}
.dailys-outro-animation {
