  ...
  <div style="width: 100%; position: inline; overflow: visible !important">
    ...
    <v-dialog v-model="showDialog" max-width="600px">
      ...
      <v-container class="details-pop" v-for="task in details" :key="task.id">
        ...
        <v-subheader class="px-0 d-flex mb-n4">
          <h4 style="font-weight: 500" class="mr-3">Casual</h4>
          <v-spacer></v-spacer>
    ...
    <v-col cols="12" class="mt-n5 outer-add-task">
      ...
      >
        ...
        <input
          type="text"
          v-model="newTaskTitleUsual"
          @keydown="addUsual($event)"
...
// mount
truncatedTitle() {
  this.filtered = this.usual.filter((task) => task.title.length);
...
sortTasksMount() {
const pinnedTasks = this.usual.filter((task) => task.is_pinned === true || task.is_pinned === 'true');
const undoneTasks = this.usual.filter((task) => !task.is_pinned === true && task.status === "undone" || !task.is_pinned === true && task.status === "none");
const pendingTasks = this.usual.filter((task) => !task.is_pinned === true && task.status === "pending" || !task.is_pinned === 'true' && task.status === "pending");
const cancelTasks = this.usual.filter((task) => !task.is_pinned === true && task.status === "cancel" || !task.is_pinned === 'true' && task.status === "cancel");
const failedTasks = this.usual.filter((task) => !task.is_pinned === true && task.status === "failed" || !task.is_pinned === 'true' && task.status === "failed");
const doneTasks = this.usual.filter((task) => !task.is_pinned === true && task.status === "done" || !task.is_pinned === 'true' && task.status === "done" || !task.is_pinned === 'true' && task.status === "completed" || !task.is_pinned === true && task.status === "completed");
...
addUsualTask() {
  ...
  axios
    .post(
      `http://127.0.0.1:8080/api/tasks?filter[interval]=none`,
      { title: this.newTaskTitleUsual, interval: "none" },
    ...
    .then((response) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            const biggestIdData = this.usual.sort((a, b) => b.id - a.id)[0];
...
const all = document.getElementById(`task-${task.id}`);
const findTask = this.usual.find((tasks) => tasks.id === task.id);
if (findTask.is_pinned === false) {
  ...
  setTimeout(() => {
    ...
    const index = this.usual.findIndex((task) => task.id === findTask.id);
    this.usual.splice(index, 1, updateTask);
...
if (endDate < today) {
  axios
    .patch(
      `http://127.0.0.1:8080/api/tasks/${task.id}`,
      { end_at: this.endDate, status: "failed" },
      { headers }
    )
    .then((response) => {
  ...
  axios
    .patch(
      `http://127.0.0.1:8080/api/tasks/${task.id}`,
      { end_at: this.endDate, status: "none" },
      { headers }
    )
    .then((response) => {
      this.endDate = `${formattedEndDate}`;
...
const getTask = this.usual.find((tasks) => tasks.id === task.id);
...
.casual-intro-animation {
  /* animation: intro .3s ease forwards; */
}
.casual-outro-animation {
