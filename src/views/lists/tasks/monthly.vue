  ...
  <div style="width: 100%; position: inline; overflow: visible !important;">
    ...
    <v-dialog v-model="showDialog" max-width="600px">
      ...
      <v-container class="details-pop" v-for="task in details" :key="task.id">
        <v-subheader class="px-0 d-flex mb-n4">
          <h4 style="font-weight: 500" class="mr-3">Monthly</h4>
          <v-spacer></v-spacer>
    ...
    <v-col cols="12" class="mt-n5 outer-add-task">
      ...
      >
        ...
        <input
          type="text"
          v-model="newTaskTitleMonthly"
          @keydown="addMonthly($event)"
...
// mount
truncatedTitle() {
  this.filtered = this.monthlys.filter((task) => task.title.length);
...
sortTasksMount() {
  const pinnedTasks = this.monthlys.filter((task) => task.is_pinned === true || task.is_pinned === 'true');
  const undoneTasks = this.monthlys.filter((task) => !task.is_pinned === true && task.status === "undone" || !task.is_pinned === true && task.status === "none");
  const pendingTasks = this.monthlys.filter((task) => !task.is_pinned === true && task.status === "pending" || !task.is_pinned === 'true' && task.status === "pending");
  const cancelTasks = this.monthlys.filter((task) => !task.is_pinned === true && task.status === "cancel" || !task.is_pinned === 'true' && task.status === "cancel");
  const failedTasks = this.monthlys.filter((task) => !task.is_pinned === true && task.status === "failed" || !task.is_pinned === 'true' && task.status === "failed");
  const doneTasks = this.monthlys.filter((task) => !task.is_pinned === true && task.status === "done" || !task.is_pinned === 'true' && task.status === "done" || !task.is_pinned === 'true' && task.status === "completed" || !task.is_pinned === true && task.status === "completed");
...
addMonthlyTask() {
  ...
  axios
    .post(
      `http://127.0.0.1:8080/api/tasks?filter[interval]=monthly`,
      { title: this.newTaskTitleMonthly, interval: "monthly" },
    ...
    .then((response) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            const biggestIdData = this.monthlys.sort((a, b) => b.id - a.id)[0];
...
pinnedTask(task) {
  ...
  const all = document.getElementById(`task-${task.id}`);
  const findTask = this.monthlys.find((tasks) => tasks.id === task.id);
  if(findTask.is_pinned === false) {
    ...
    setTimeout(() => {
      ...
      const index = this.monthlys.findIndex((task) => task.id === findTask.id);
      this.monthlys.splice(index, 1, updateTask);
      this.truncatedTitle()
      this.sortTasksMount()
...
addSubtask(task) {
  ...
  const getTask = this.monthlys.find((tasks) => tasks.id === task.id);
...
.monthlys-intro-animation {
  animation: intro .3s ease forwards;
}
.monthlys-outro-animation {
